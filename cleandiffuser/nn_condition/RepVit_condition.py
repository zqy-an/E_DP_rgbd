...
class RepVitObsCondition(BaseNNCondition):
    """
    Processes RGB and low-dimensional inputs into embeddings for diffusion policy.
    """

    def __init__(self, shape_meta, emb_dim=256, resize_shape=None,
                 share_rgb_model=False, image_channel=16, use_seq=False, keep_horizon_dims=False):
        super().__init__()
        self.rgb_keys, self.low_dim_keys = [], []
        self.key_shape_map = {}
        ...
        # RGB model configuration
        cfgs = [
            [3, 2, 40, 1, 0, 1], [3, 2, 40, 0, 0, 1], [3, 2, 80, 0, 0, 2],
            [3, 2, 80, 1, 0, 1], [3, 2, 80, 0, 0, 1], [3, 2, 160, 0, 1, 2],
            [3, 2, 160, 1, 1, 1], [3, 2, 160, 0, 1, 1], [3, 2, 160, 1, 1, 1],
            [3, 2, 160, 0, 1, 1], [3, 2, 160, 1, 1, 1], [3, 2, 160, 0, 1, 1],
            [3, 2, 160, 1, 1, 1], [3, 2, 160, 0, 1, 1], [3, 2, 160, 0, 1, 1],
            [3, 2, 320, 0, 1, 2], [3, 2, 320, 1, 1, 1],
        ]
        rgb_model = RepViT(cfgs, image_channel=image_channel, num_classes=emb_dim, distillation=False)

        # Handle shared RGB model
        if share_rgb_model:
        ...
        for key, attr in shape_meta['obs'].items():
            ...
            if attr.get('type', 'low_dim') == 'rgb':
                ...
                self.key_transform_map[key] = self._get_rgb_transform(resize_shape, key, shape)
            else:
        ...
        self.mlp = nn.Sequential(nn.Linear(self.output_shape(), emb_dim), nn.LeakyReLU(), nn.Linear(emb_dim, emb_dim))

    def _get_rgb_transform(self, resize_shape, key, shape):
        if resize_shape:
            h, w = resize_shape[key] if isinstance(resize_shape, dict) else resize_shape
            return torchvision.transforms.Resize(size=(h, w))
        return nn.Identity()
    ...
    def multi_image_forward(self, obs_dict):
        ...
        if 'rgb' in self.key_model_map:
            ...
            imgs = torch.cat(imgs, dim=0)  # Combine all RGB inputs
            feature = self.key_model_map['rgb'](imgs)
            batch_size = feature.size(0) // len(self.rgb_keys)
            feature = feature.view(len(self.rgb_keys), batch_size, -1).transpose(0, 1).reshape(batch_size, -1)
    ...
    def forward(self, obs_dict, mask=None):
        batch_size, seq_len = self._get_batch_size(obs_dict)
        features = self.multi_image_forward(obs_dict)
        result = self.mlp(features)
        if self.use_seq:
            result = result.view(batch_size, seq_len, -1) if self.keep_horizon_dims else result.view(batch_size, -1)
        return result

    @torch.no_grad()
    def output_shape(self):
        dummy_obs = {k: torch.zeros((8, 1) + shape if self.use_seq else (8,) + shape) for k, shape in self.key_shape_map.items()}
        return self.multi_image_forward(dummy_obs).shape[-1]

    def _get_batch_size(self, obs_dict):
        tensor = next(iter(obs_dict.values()))
...
if __name__ == "__main__":
    ...
    im = RepVitObsCondition(shape_meta, emb_dim=256, resize_shape=resize_shape,
